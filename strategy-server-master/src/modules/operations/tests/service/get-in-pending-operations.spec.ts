...
describe('getInPendingOperations', () => {
  ...
  describe('getInPendingLowRiskStrategyOperation tests', () => {
    it('Нет операций в статусе PENDING', async () => {
      const result = await operationsService.getInPendingLbfOperation({
        type: OPERATION_TYPE.STRATEGY_RUN,
  ...
  describe('Проверяем, что CLAIM_RUN не вернется', () => {
    ...
    it('Ничего не вернется, так как тип не STRATEGY_RUN', async () => {
      const result = await operationsService.getInPendingLbfOperation({
        type: OPERATION_TYPE.STRATEGY_RUN,
        strategyId: operationMock.strategyId,
      });
  ...
  describe('Проверяем, что операция в другом статусе не вернтся', () => {
    ...
    it('Ничего не вернется, так как статус не PENDING', async () => {
      const result = await operationsService.getInPendingLbfOperation({
        type: OPERATION_TYPE.STRATEGY_RUN,
        strategyId: operationMock.strategyId,
      });
