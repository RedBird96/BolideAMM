...
describe('InProgressOperation', () => {
  ...
  describe('Проверяем, что меняется статус на IN_PROGRESS для стратегии', () => {
    let opt;
    ...
    it('Вернется 1 запись со статусом IN_PROGRESS', async () => {
      await operationsService.inProgressOperation({
        id: opt.id,
      ...
      expect(result.status).toBe(OPERATION_STATUS.IN_PROGRESS);
