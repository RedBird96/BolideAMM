...
async function main() {
  ...
  const logicForceInit = process.env.LBL_LOGIC_FORCE_INIT;
  const strategyForceInit = process.env.LBL_STRATEGY_FORCE_INIT;
  const logicUpgrade = Boolean(process.env.LBL_LOGIC_FORCE_UPGRADE);
  const strategyUpgrade = Boolean(process.env.LBL_STRATEGY_FORCE_UPGRADE);
  const version = "1.1.0";
