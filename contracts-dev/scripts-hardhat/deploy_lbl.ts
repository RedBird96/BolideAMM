...
async function main() {
  ...
  let logicImpl = "";
  dotenv.config();
  console.log("- LendBorrowLend migration -");

  // @ts-ignore
  const addresses = ADDRESS_COLLECTION[network.config.addressesSet];
  const logicAddress = process.env.LBL_LOGIC_PROXY_ADDRESS!;
  const strategyAddress = process.env.LBL_STRATEGY_PROXY_ADDRESS!;
  const logicForceInit = yn(process.env.LBL_LOGIC_FORCE_INIT);
  const strategyForceInit = yn(process.env.LBL_STRATEGY_FORCE_INIT);
  const logicUpgrade = yn(process.env.LBL_LOGIC_FORCE_UPGRADE);
  const strategyUpgrade = yn(process.env.LBL_STRATEGY_FORCE_UPGRADE);
  ...
  if (!strategyAddress) {
    ...
    strategy = LendBorrowLendStrategy__factory.connect(
      strategyAddress,
      deployer
    );
  }
  ...
  if (strategyInit) {
    ...
    if (addresses.Token.BUSD) {
      tx = await strategy
        .connect(deployer)
        .init(addresses.Token.BUSD.Underlying, addresses.Token.BUSD.Ola);
      await tx.wait(1);
    }
    ...
    if (addresses.Token.USDT) {
      tx = await strategy
        .connect(deployer)
        .init(addresses.Token.USDT.Underlying, addresses.Token.USDT.Ola);
      await tx.wait(1);
    }
    ...
    if (addresses.Token.USDC) {
      tx = await strategy
        .connect(deployer)
        .init(addresses.Token.USDC.Underlying, addresses.Token.USDC.Ola);
      await tx.wait(1);
    }

    if (addresses.Token.BANANA && addresses.Token.BNB) {
      tx = await strategy
        .connect(deployer)
