...
async function main() {
  ...
  const logicForceInit = yn(process.env.LBF_LOGIC_FORCE_INIT);
  const strategyForceInit = yn(process.env.LBF_STRATEGY_FORCE_INIT);
  const logicUpgrade = yn(process.env.LBF_LOGIC_FORCE_UPGRADE);
  const strategyUpgrade = yn(process.env.LBF_STRATEGY_FORCE_UPGRADE);
  ...
  if (strategyInit) {
    ...
    if (addresses.Token.BUSD) {
      tx = await strategy
        .connect(deployer)
        .addVToken(addresses.Token.BUSD.Underlying, addresses.Token.BUSD.Venus);
      await tx.wait(1);
    }
    ...
    if (addresses.Token.USDT) {
      tx = await strategy
        .connect(deployer)
        .addVToken(addresses.Token.USDT.Underlying, addresses.Token.USDT.Venus);
      await tx.wait(1);
    }
    ...
    if (addresses.Token.DAI) {
      tx = await strategy
        .connect(deployer)
        .addVToken(addresses.Token.DAI.Underlying, addresses.Token.DAI.Venus);
      await tx.wait(1);
    }
    ...
    if (addresses.Token.USDC) {
      tx = await strategy
        .connect(deployer)
        .addVToken(addresses.Token.USDC.Underlying, addresses.Token.USDC.Venus);
      await tx.wait(1);
    }
  }
